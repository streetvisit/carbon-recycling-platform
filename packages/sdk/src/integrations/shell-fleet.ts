...
export interface ShellFleetVehicleData {
  id: string;
  date: string;
  mileage: number; fuel_consumption: number;
}

export class ShellFleetIntegration extends BaseIntegration {
  protected baseUrl = 'https://api.shell.com/fleet/v1';
  
  constructor(config: IntegrationConfig) {
    super({
      ...config,
      type: IntegrationType.TRANSPORT,
      provider: 'Shell Fleet Solutions',
