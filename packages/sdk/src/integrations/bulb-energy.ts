...
export interface BulbEnergyMeterReading {
  meter_id: string;
  reading_date: string;
...
export interface BulbEnergyAccount {
  account_number: string;
  business_name?: string;
  meters: string[];
}

export class BulbEnergyIntegration extends BaseIntegration {
  protected baseUrl = 'https://api.bulb.co.uk/v1';
  
  constructor(config: IntegrationConfig) {
    super({
      ...config,
      type: IntegrationType.UTILITY,
      provider: 'Bulb Energy',
