...
export interface EmersonDeltavProductionData {
  id: string;
  date: string;
  production_volume: number; energy_usage: number;
}

export class EmersonDeltavIntegration extends BaseIntegration {
  protected baseUrl = 'https://api.emerson.com/v1';
  
  constructor(config: IntegrationConfig) {
    super({
      ...config,
      type: IntegrationType.MANUFACTURING,
      provider: 'Emerson DeltaV',
